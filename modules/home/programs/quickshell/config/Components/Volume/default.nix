{pkgs}:
pkgs.writeShellApplication {
  name = "quickshell-volume-status";
  runtimeInputs = [pkgs.gawk pkgs.jq pkgs.wireplumber];
  text = ''
    emit_status() {
      volume="$(${pkgs.wireplumber}/bin/wpctl get-volume @DEFAULT_AUDIO_SINK@ 2>/dev/null)" || {
        ${pkgs.jq}/bin/jq -cn '{text: "vol unavailable", tooltip: "Audio sink unavailable", class: "unavailable", percentage: 0}'
        return
      }

      if [[ "$volume" == *"[MUTED]"* ]]; then
        ${pkgs.jq}/bin/jq -cn '{text: "vol muted", tooltip: "Volume: muted", class: "muted", percentage: 0}'
        return
      fi

      percentage="$(awk '{printf "%.0f", $2 * 100}' <<< "$volume")"
      ${pkgs.jq}/bin/jq -cn --arg text "vol ''${percentage}%" --argjson percentage "$percentage" \
        '{text: $text, tooltip: ("Volume: " + ($percentage | tostring) + "%"), class: "volume", percentage: $percentage}'
    }
    emit_status
  '';
}
