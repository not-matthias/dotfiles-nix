...
- Use the version-supported presymbolication option for saved captures so gathered symbols are embedded in the profile JSON. Current Samply versions use `--unstable-presymbolicate`, while some versions use `--presymbolicate`; check `samply record --help` and use whichever is supported.
- Retain any sidecar files generated by the presymbolication option next to the profile.
...
```bash
samply record --save-only --unstable-presymbolicate -o /tmp/profile.json.gz -- ./target/release/app --input data.json
samply record --save-only --unstable-presymbolicate -o /tmp/profile.json.gz -- cargo test --release benchmark_name -- --nocapture
```
...
Increase the iteration count until the profile contains enough samples on the workload thread. Avoid wrapping the command in shell loops unless the shell overhead is part of what the user wants.
...
```bash
samply record --save-only --unstable-presymbolicate -o /tmp/system-profile.json.gz --duration 30 --all
```
...
The presymbolication option may create sidecar files containing gathered symbol data. Keep those files next to the profile when moving or archiving it.
...
```bash
samply record --save-only --unstable-presymbolicate --profile-name baseline -o /tmp/baseline.json.gz -- <benchmark-command>
# apply one change
samply record --save-only --unstable-presymbolicate --profile-name candidate -o /tmp/candidate.json.gz -- <benchmark-command>
```
...
- **Symbols are missing**: Rebuild with debug info, keep the exact binary, use `--symbol-dir`, or pass `--binary` to the analyzer. For future offline analysis, use the version-supported presymbolication option (`--unstable-presymbolicate` or `--presymbolicate`) and retain its sidecar files.
