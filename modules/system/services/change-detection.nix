...
in {
  options.services.change-detection.enable = lib.mkEnableOption "Enable Change Detection";

  config = lib.mkIf cfg.enable {
    virtualisation.oci-containers.containers = {
      "playwright-chrome".image = "docker.io/browserless/chrome:1.61.1-chrome-stable";
