...
### Desktop Environment
- **Primary**: Niri with Eww status bar and Vicinae launcher
...
- **Nix configuration preference**: Use dotted paths for single-attribute sets, such as `_1password.enable = true;`. Keep nested sets for multiple related fields, such as `helix = { enable = true; compat.enable = true; };`.
...
- **Source of truth for shared skills**: `modules/home/programs/cli-agents/shared/skills/` in the dotfiles-nix repo (symlinked into each agent's skills directory, e.g. `~/.omp/agent/skills/`, via Nix; not installed for Claude Code)
